...
@Composable
fun NoteSearch(navController: NavController,query:String,viewModel: NoteViewModel = hiltViewModel()) {
    val getSearch by viewModel.searchNote.collectAsState()
    ...
    LazyColumn(
        ...
        items(getSearch) { item ->
            NoteSearchCard(
                title = item.title,
                content = item.content,
                onEdit = {navController.navigate("Update/${item.id}")
                            viewModel.updateNote(item)
                         },
                onDelete = {viewModel.deleteNote(id = item.id)
...
@Composable
fun NoteSearchCard(title: String, content: String,onEdit:()->Unit,onDelete:()->Unit) {
    Card(
        modifier = Modifier
            .padding(8.dp)
